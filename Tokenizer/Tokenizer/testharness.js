...
const exetools_1 = require("./exetools");
...
function runOneTestcase() {
    ...
    let outputfiles = testcase["outputfiles"];
    let stdin = testcase["input"];
    let group = testcase["bonus"];
    ...
    exetools_1.assembleAsync(asmfile, objfile, (err) => {
        if (err) {
            reportError("Assemble error: " + err);
            return;
        }
        console.log("Assemble OK");
        exetools_1.linkAsync(objfile, exefile, (err) => {
            if (err) {
                reportError("Link error: " + err);
                return;
            }
            console.log("Link OK");
            let ex = "." + path.sep + exefile;
            let opts = {
